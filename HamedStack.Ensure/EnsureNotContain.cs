...
{
    /// <summary>
    ///     Ensures that the value does not contain a specific element. For collections, it checks if the element is not
    ///     present; for enums, it checks if the enum value does not match the element.
    /// </summary>
    ...
    {
        if (!EqualityComparer<T>.Default.Equals(value, element)) return value;

        if (value is IEnumerable<T> enumerable)
        {
            if (!enumerable.Contains(element)) return value;
        }
