...
public static partial class EnsureExtensions
{
    ...
    {
        var isLessThanMin = minBoundary == BoundaryType.Exclusive ? value.CompareTo(minValue) <= 0 : value.CompareTo(minValue) < 0;
        var isGreaterThanMax = maxBoundary == BoundaryType.Exclusive ? value.CompareTo(maxValue) >= 0 : value.CompareTo(maxValue) > 0;
    ...
    /// <summary>
    /// Ensures that the value falls within the specified range inclusively or exclusively, using a custom exception creator delegate.
    /// </summary>
    /// <typeparam name="T">The type of the value to check.</typeparam>
    /// <param name="value">The value to check.</param>
    /// <param name="minValue">The minimum allowed value.</param>
    ...
    /// Thrown when the value is outside the specified range, as created by the <paramref name="exceptionCreator"/> delegate.
    /// </exception>
    public static T EnsureBetween<T>(
        this T value,
        T minValue,
